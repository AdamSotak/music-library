...
export default function BarcodeScanner() {
	...
	const captureAndScan = async () => {
		...
		const tryBothOrientations = async () => {
			...
			if (normalBlob) {
				console.log(
					"Trying normal orientation, blob size:",
					normalBlob.size,
					"bytes",
				)
			...
			if (mirroredBlob) {
				console.log(
					"Trying mirrored orientation, blob size:",
					mirroredBlob.size,
					"bytes",
				)
		...
		try {
			...
			if (result?.track_id) {
				...
				setError(
					"Could not decode barcode. Try adjusting the angle or distance.",
				)
				setScanning(false)
			}
		} catch (err: any) {
			...
			{!isScanning ? (
				...
				<div className="flex flex-col items-center gap-4 w-full">
					<div className="relative w-full max-w-2xl aspect-video bg-black rounded-lg overflow-hidden">
						...
						{videoReady && (
							<div
								className={`absolute inset-0 border-4 pointer-events-none transition-colors duration-300 ${
									success
