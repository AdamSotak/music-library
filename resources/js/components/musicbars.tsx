...
export default function MusicBarcode({ trackId, trackName }: MusicBarcodeProps) {
    ...
    return (
        <>
            ...
            >
                {loading ? (
                    <svg className="min-w-7 min-h-7 animate-spin" fill="gray" viewBox="0 0 24 24">
                        <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" fill="none"></circle>
                        <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                    </svg>
                ) : (
                    <svg className="min-w-7 min-h-7 transition-colors duration-300 group-hover:fill-white" fill="gray" viewBox="0 0 24 24">
            ...
            <Dialog open={isOpen} onOpenChange={setIsOpen}>
                <DialogContent className="max-w-3xl bg-zinc-900 border-white/10">
                    <DialogHeader>
                        <DialogTitle className="text-white text-xl">Music Code - {trackName}</DialogTitle>
                    </DialogHeader>
