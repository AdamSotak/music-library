...
export default function MusicBarcode({ trackId, trackName }: MusicBarcodeProps) {
	...
	return (
		<>
			<Button onClick={() => setIsOpen(true)} variant="spotifyTransparent" size="icon" aria-label="Show QR">
			...
			<Dialog open={isOpen} onOpenChange={setIsOpen}>
				<DialogContent className="max-w-xl bg-zinc-900 border-white/10">
					<DialogHeader>
						<DialogTitle className="text-white text-xl">Open on phone — {trackName}</DialogTitle>
