...
export function useAudioEffects() {
	...
	const processAudio = useCallback(async (audioBuffer: AudioBuffer) => {
		...
		try {
			...
			// Convert pitch from linear to semitones if needed
			const pitchScale = effects.pitch
			const tempoRatio = 1.0 / effects.tempo
			...
			while (read < channelBuffers[0].length) {
				for (let i = 0; i < channelBuffers.length; i++) {
					rbApi.memWrite(channelDataPtr[i], channelBuffers[i].subarray(read, read + samplesRequired))
				}
				...
				while (true) {
					...
					for (let i = 0; i < channelDataPtr.length; i++) {
						if (write + recv <= outputBuffers[i].length) {
							outputBuffers[i].set(rbApi.memReadF32(channelDataPtr[i], recv), write)
						}
					}
					write += recv
				}
			...
			for (let channel = 0; channel < channels; channel++) {
				...
				const processedData = outputBuffers[channel].subarray(0, outputBuffer.length)
	...
	}, [effects.tempo, effects.pitch, initializeAudio])

	// Load and decode audio file
	const loadAudioFile = useCallback(async (audioUrl: string): Promise<AudioBuffer> => {
		if (!audioContextRef.current) {
			await initializeAudio()
		}
	...
	const setTempo = useCallback((tempo: number) => {
		setEffects(prev => ({ ...prev, tempo: Math.max(0.5, Math.min(2.0, tempo)) }))
	}, [])

	// Update pitch
	const setPitch = useCallback((pitch: number) => {
		setEffects(prev => ({ ...prev, pitch: Math.max(0.5, Math.min(2.0, pitch)) }))
	}, [])
